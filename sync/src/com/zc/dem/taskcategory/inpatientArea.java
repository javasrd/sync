...
public class inpatientArea implements Job
{
    /** 日志类 */
    private static final Logger log = LoggerFactory.getLogger(inpatientArea.class);
    ...
    {
        log.info("inpatientArea execute");
        // 同步模式
        ...
        else if (SYNCHRONIZATION_MODE_VIEW.equals(synchronizationMode))
        {
            try
            {
                List<InpatientAreaBean> inparentAreaList =
                    StartUpServlet.inpatientAreaByHisService.qryInpatientAreaList();
                
                if (inparentAreaList != null && inparentAreaList.size() > 0)
                {
                    StartUpServlet.inpatientAreaService.synData(inparentAreaList);
                }
        ...
        throws JobExecutionException, HttpException, IOException
    {
        ...
        try
        {
            result = StartUpServlet.getSynDataForSZ.synInPatientArea();
        }
        catch (Exception e)
        {
            log.error(e.getMessage(), e);
    ...
    {
        //获取对应任务的脚本文件路径
        String patientShell = RestfulRetCodeConfiguration.getStringProperty(DemConstant.actionTypeDesc.inpatientArea);
