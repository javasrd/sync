...
public class medicamentDict implements Job
{
    /** 日志类 */
    private static final Logger log = LoggerFactory.getLogger(medicamentDict.class);
    ...
    {
        log.info("medicamentDict execute");
        // 同步模式
        ...
        else if (SYNCHRONIZATION_MODE_VIEW.equals(synchronizationMode))
        {
            try
            {
                List<Medicaments> medicamentList = StartUpServlet.medicamentsByHisService.qryMedicamentList();
                ...
                if (medicamentList != null && medicamentList.size() > 0)
                {
                    ...
                    StartUpServlet.medicamentsService.synData(medicamentList);
                }
        ...
        throws JobExecutionException, HttpException, IOException
    {
        ...
        try
        {
            result = StartUpServlet.getSynDataForSZ.synDrug();
        }
        catch (Exception e)
        {
            log.error(e.getMessage(), e);
    ...
    {
        //获取对应任务的脚本文件路径
        String patientShell = RestfulRetCodeConfiguration.getStringProperty(DemConstant.actionTypeDesc.medicamentDict);
