...
public class patient implements Job
{
    /** 日志类 */
        ...
        throws JobExecutionException
    {
        ...
        else if (SYNCHRONIZATION_MODE_VIEW.equals(synchronizationMode))
        {
            try
            {
                List<PatientBean> patientList = StartUpServlet.patientByHisService.qryPatientList();
                
                if (patientList != null && patientList.size() > 0)
                {
                    StartUpServlet.patientService.synData(patientList);
                }
        ...
        throws JobExecutionException, HttpException, IOException, ParseException
    {
        ...
        try
        {
            result = StartUpServlet.getSynDataForSZ.synPatient();
        }
        catch(Exception e)
        {
            log.error(e.getMessage(), e);
    ...
    {
        //获取对应任务的脚本文件路径
        String patientShell = RestfulRetCodeConfiguration.getStringProperty(DemConstant.actionTypeDesc.patient);
