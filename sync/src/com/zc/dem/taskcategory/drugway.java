...
public class drugway implements Job
{
    /** 日志类 */
    private static final Logger log = LoggerFactory.getLogger(drugway.class);
        ...
        throws JobExecutionException
    {
        ...
        else if (SYNCHRONIZATION_MODE_VIEW.equals(synchronizationMode))
        {
            try
            {
                List<DrugWayBean> drugWayList = StartUpServlet.drugWayByHisService.qryDrugWayList();
                
                if (drugWayList != null && drugWayList.size() > 0)
                {
                    StartUpServlet.drugWayService.synData(drugWayList);
                }
        ...
        throws JobExecutionException, HttpException, IOException
    {
        ...
        try
        {
            result = StartUpServlet.getSynDataForSZ.synDrugWay();
        }
        catch (Exception e)
        {
            log.error(e.getMessage(), e);
    ...
    {
        //获取对应任务的脚本文件路径
        String patientShell = RestfulRetCodeConfiguration.getStringProperty(DemConstant.actionTypeDesc.drugway);
