...
public class excuteRecord implements Job
{
    /** 日志类 */
    private static final Logger log = LoggerFactory.getLogger(excuteRecord.class);
        ...
        throws JobExecutionException
    {
        ...
        if (SYNCHRONIZATION_MODE_VIEW.equals(synchronizationMode))
        {
            try
            {
                List<ExcuteRecordBean> recordList = StartUpServlet.synYdRecordByHisService.qryYdRecordList(null);
                ...
                if (recordList != null && recordList.size() > 0)
                {
                    ...
                    StartUpServlet.synYdRecordService.synData(recordList);
                }
        ...
        throws JobExecutionException, HttpException, IOException
    {
        ...
        try
        {
            result = StartUpServlet.getSynDataForSZ.synExcuteRecord();
        }
        catch (Exception e)
        {
            log.error(e.getMessage(), e);
    ...
    {
        //获取对应任务的脚本文件路径
        String patientShell = RestfulRetCodeConfiguration.getStringProperty(DemConstant.actionTypeDesc.excuteRecord);
